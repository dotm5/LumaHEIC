...
LumaHEIC 是一个开源、纯浏览器端运行的 Apple HDR gain map HEIC 生成与调试工具。它可以把普通 SDR 图片，或自定义的 SDR base + gain map 组合，导出为 Apple Photos 可识别的 HDR HEIC 文件。

不同于只提供单个“HDR 强度”滑条的一键式工具，LumaHEIC 更关注可控性：你可以调整 headroom、HDR 强度、高光响应、保护强度、gain map 分辨率，并查看 gain map、HDR 参考预览和导出 HEIC 的元数据验证结果。

所有处理都在本地浏览器中完成。图片不会上传到服务器；解码、gain map 生成、预览渲染和 HEIC 编码都运行在 Web Worker 与 `libheif + x265` WASM 编码器中。

在线使用：[https://dotm5.github.io/LumaHEIC/](https://dotm5.github.io/LumaHEIC/)

目标：让 Apple HDR / EDR 图片生成不再是黑盒，而是一个可解释、可控制、可调试、可复现的开放工作流。

生成的 HDR 效果是合成结果。LumaHEIC 不会恢复源图中本来不存在的真实场景 HDR 信息。

## 为什么做 LumaHEIC？

Apple 设备具备很强的 HDR / EDR 显示能力，也支持基于 gain map 的照片工作流，但生成一个能被 Apple Photos 识别的 HDR HEIC 文件并不透明。很多图片工具更偏向一键式体验，用户能看到最终观感，却很难确认 gain map 分辨率、headroom、辅助图像、XMP 元数据和 HEIF item reference 到底是什么状态。

LumaHEIC 的重点不是简单套一个 HDR 效果，而是提供一套开放的 Apple HDR gain map authoring workflow：你可以理解它生成了什么，调参数，导出 HEIC，再检查文件结构是否符合预期。
...
### 1. 从 SDR 图片生成 Apple HDR HEIC

使用一张 JPEG 或 PNG 作为输入。LumaHEIC 会在浏览器本地生成 synthetic Apple-style HDR gain map，提供 SDR 基图 / gain map / HDR 参考预览，并导出 Apple Photos 可识别的 HDR gain-map HEIC。

### 2. 从 Base + Gain Map 生成 HEIC

使用一张 SDR base 图片和一张你自己准备的灰度 gain map。这个模式适合更可控的创作流程、格式实验，以及调试 base + gain-map 图像处理管线。

### 3. 不只用一个强度滑条，而是调 HDR 参数

项目提供预设，但不会把结果锁死在预设里。你可以继续调整 headroom、HDR 强度、曝光、高光、白场、阴影保护、饱和度保护、肤色保护、glow、边缘平滑、小高光保留和 gain map 分辨率。
...
LumaHEIC 会写入 Apple HDR gain-map auxiliary image、XMP metadata，以及 native encoder 使用的 MakerApple HDR metadata 路径。仓库内提供本地验证脚本，用来检查导出后的辅助图像、元数据和 HEIF item reference。

## 适合谁使用？

- 想把普通图片转成 Apple Photos HDR HEIC 的用户。
- 希望获得比一键 HDR 滤镜更多控制的创作者、摄影和调色用户。
- 研究 Apple HDR / EDR、gain map 和 HEIF auxiliary image 的开发者。
